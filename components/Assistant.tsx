...
import { LEGAL_PROMPT } from './legalPrompt';
...
const Assistant: React.FC = () => {
  ...
  // --------- NOUVEAU : construction de prompt “pro juridique” ----------

  const buildUserPrompt = (rawInput: string) => {
    // Petit bloc qui décrit le contexte (langue, branche, pays…)
    const configBlock = `
LANGUAGE: "${language}"
LEGAL_DOMAIN: "${domain}"
TARGET_USER: "Lawyer / Legal professional"
JURISDICTION: "France"
DETAIL_LEVEL: "${useThinking ? 'detailed analysis' : 'standard analysis'}"
TONE: "neutral and professional"
`;

    // On assemble : cerveau juridique + config + question de l’utilisateur
    const fullPrompt = `
${LEGAL_PROMPT}
...
`;
  ...
  const handleSend = async () => {
    ...
    try {
      // historique pour Gemini (comme avant)
      ...
      const promptWithConfig = buildUserPrompt(input);
      ...
      const errorMessage: Message = {
        ...
        role: 'model',
        content:
          "Une erreur critique est survenue. Veuillez vérifier votre connexion ou l'état du service.",
        timestamp: new Date()
      };
  ...
  // Helper pour mettre en forme le texte juridique
  const renderLegalContent = (content: string) => {
    const lines = content.split('\n');
    return lines.map((line, i) => {
      const isHeader =
        /^\d+\.\s/.test(line) ||
        line.includes('Résumé') ||
        line.includes('Analyse') ||
        line.includes('Recommandations');
      const isCritical =
        line.toLowerCase().includes('avocat') ||
        line.toLowerCase().includes('risque');
      ...
      if (isHeader) {
        return (
          <h4
            key={i}
            className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-700 mt-6 mb-3 border-l-2 border-blue-600 pl-3"
          >
            {line}
          </h4>
        );
      ...
      if (isCritical) {
        return (
          <div
            key={i}
            className="my-4 p-4 bg-red-50 border border-red-100 rounded-xl flex gap-3 items-start"
          >
      ...
      if (isGeneral) {
        return (
          <div
            key={i}
            className="my-4 p-4 bg-slate-50 border border-slate-200 rounded-xl flex gap-3 items-start"
          >
      ...
      return (
        <p
          key={i}
          className="text-sm text-slate-700 leading-relaxed mb-2 font-serif-legal"
        >
          {line}
        </p>
      );
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-12 bg-[url('https://www.transparenttextures.com/patterns/clean-gray-paper.png')]">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center max-w-lg mx-auto py-20">
            ...
            <h3 className="font-serif-legal text-2xl font-bold text-slate-900 mb-4 tracking-tight">
              Poste de Consultation JurisIA
            </h3>
            <p className="text-slate-500 text-sm font-light leading-relaxed mb-10">
              Engagement de la session sous protocole sécurisé. Vos échanges
              sont chiffrés et isolés au sein de votre environnement de travail
              souverain.
            </p>
            <div className="grid grid-cols-1 gap-3 w-full max-w-xs">
              <button
                onClick={() =>
                  setInput(
                    "Analyser les risques d'une clause de résiliation unilatérale sans préavis."
                  )
                }
                className="p-4 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:border-blue-500 hover:text-blue-600 transition-all text-left flex items-center gap-3"
              >
              ...
              <button
                onClick={() =>
                  setInput(
                    'Proposer une reformulation pour une clause de force majeure incluant les cyberattaques.'
                  )
                }
                className="p-4 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-600 hover:border-blue-500 hover:text-blue-600 transition-all text-left flex items-center gap-3"
              >
                <i className="fa-solid fa-pen-fancy text-blue-500"></i> Rédaction
                de clause
              </button>
            </div>
          </div>
        ...
        {messages.map((m) => (
          ...
          >
            <div
              className={`relative max-w-[90%] md:max-w-[80%] ${
                m.role === 'user'
            ...
            >
              {m.role === 'model' && (
                <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="block text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">
                        Rapport d'Expertise Digitale
                      </span>
                      <span className="block text-[8px] font-bold text-slate-400 uppercase tracking-widest">
                        Généré en {language} • JurisIA v3.0
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
              ...
              <div
                ...
                } text-[8px] font-black text-slate-300 uppercase tracking-widest`}
              >
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-white border border-slate-100 rounded-[2rem] p-8 shadow-xl flex items-center gap-6 animate-pulse">
              ...
              <div>
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-[0.4em] mb-1">
                  Traitement Cognitif
                </p>
                <p className="text-[9px] text-slate-400 font-bold uppercase">
                  Sourcing, Analyse et Structuration en cours...
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mt-4 px-4">
            <p className="text-[9px] font-bold text-slate-400 flex items-center gap-2 italic">
              <i className="fa-solid fa-lock text-emerald-500"></i> Confidentialité
              client garantie par isolation cryptographique
            </p>
            <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">
              JurisIA Terminal Alpha
            </p>
          </div>
        </div>
      </div>
